...
describe('buildDraftInvoice — negotiated rates and terms', () => {
  // Puente bills BOTH ways: a recurring per-month subscription, and services
  // priced by negotiation per partner. A shared catalogue alone cannot express
  // the second, and quietly charging a partner the list price for something
  // they negotiated is the kind of error that ends a relationship.
  it("uses the organization's negotiated rate over the standard catalogue", () => {
    const draft = buildDraftInvoice({
      organization: {
        shortCode: 'wof', plan: 'partner',
        negotiatedRates: { 'custom-form-build': 25000 },
      },
      period: PERIOD,
      rateCard: RATE_CARD,
      services: [{ code: 'custom-form-build', quantity: 1 }],
    });
    const service = draft.lineItems.find((l) => l.kind === 'service');
    expect(service.amount).toBe(25000);
    expect(service.negotiated).toBe(true);
  });

  it('marks a catalogue-priced service as NOT negotiated', () => {
    const draft = buildDraftInvoice({
      organization: { shortCode: 'wof', plan: 'partner' },
      period: PERIOD,
      rateCard: RATE_CARD,
    ...
    expect(draft.lineItems.find((l) => l.kind === 'service').negotiated).toBe(false);
  });

  it('accepts a negotiated service the standard catalogue does not price at all', () => {
    // A bespoke engagement has no list price by definition. Refusing it would
    // force a fake catalogue entry for every one-off piece of work.
    const draft = buildDraftInvoice({
      organization: {
        shortCode: 'wof', plan: 'partner',
        negotiatedRates: { 'bespoke-integration': 120000 },
      },
      period: PERIOD,
      rateCard: RATE_CARD,
      services: [{ code: 'bespoke-integration', quantity: 1 }],
    });
    expect(draft.unbillable).toBe(false);
    expect(draft.subtotal).toBe(25000 + 120000);
  });

  it('carries net terms so the due date is not invented downstream', () => {
    const draft = buildDraftInvoice({
      organization: { shortCode: 'wof', plan: 'partner' },
      period: PERIOD,
      rateCard: { ...RATE_CARD, netTermsDays: 30 },
    });
    expect(draft.netTermsDays).toBe(30);
  });

  it('marks the plan line as recurring, and services as one-off', () => {
    // The subscription recurs monthly; a service does not. Stripe needs to know
    // which is which, and so does anyone reading the draft.
    const draft = buildDraftInvoice({
      organization: { shortCode: 'wof', plan: 'partner' },
      period: PERIOD,
      rateCard: RATE_CARD,
      services: [{ code: 'data-cleanup', quantity: 1 }],
    });
    expect(draft.lineItems.find((l) => l.kind === 'plan').recurring).toBe(true);
    expect(draft.lineItems.find((l) => l.kind === 'service').recurring).toBe(false);
  });
});
