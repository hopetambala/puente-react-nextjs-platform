...
const ResetLogin = () => {
    ...
    const retrieveUser = (username) =>
        queryUser(username).then((user) => sendMessage(user, notificationType))

    const onSubmit = (data) => {
        const { usernameV } = data
        return retrieveUser(usernameV)
            .then(() => toast(<Toast text="Request Sent!" />))
            .catch(async (e) => {
            ...
            <div className={styles.paper}>
                <Card padding="extraLarge">
                    ...
                    <Stack spacing="medium" fill>
                        <Button
                            intent={
                                notificationType === 'email' ? 'primary' : ''
                            }
