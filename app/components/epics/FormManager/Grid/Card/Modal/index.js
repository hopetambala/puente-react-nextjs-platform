...
const Modl = ({ open, handleClose, row, workflows }) => {
  ...
  const handleChange = (event) => {
    if (newWorksflowsAssigned.includes(event.target.value)) {
      setNewWorkflowsAssigned(
        newWorksflowsAssigned.filter((val) => val !== event.target.value)
      );
    } else {
      setNewWorkflowsAssigned(
        newWorksflowsAssigned.concat([event.target.value])
      );
    }
  };
  ...
  return (
    ...
    >
      <div className={styles.paper}>
        <h3 id="simple-modal-description">Current Workflows Assigned:</h3>
        ...
        <FormControl component="fieldset">
          ...
          <FormGroup>
            ...
            <FormControlLabel
              value="newWorkflow"
              control={<Checkbox onChange={handleNewWorkflow} />}
              label="New Workflow"
            />
            {newWorkflow === true && (
              <TextField
