...
const Loop = (props) => {
    ...
    const [repeatQuestionsExceeded, setRepeatQuestionsExcceeded] =
        useState(false)
    ...
    useEffect(() => {
        const loopFields = formItems.filter(
            (element) => element.fieldType === 'loop'
        )
                ...
                for (let i = index - value; i < index; i += 1) {
                    if (i >= 0) {
                        ...
                        if (copiedFormItems[i].fieldType !== 'geolocation') {
                            ...
                            repeatItems = repeatItems.concat(
                                copiedFormItems[i].label
                            )
                        } else if (
                            copiedFormItems[i].fieldType !== 'geolocation'
                        ) {
            ...
            {(item.fieldType === 'loop' ||
                ...
                <div key={item.id}>
                    ...
                    <input
                        className={styles.input}
                        type="text"
                        value={item.numberQuestionsToRepeat || ''}
                        id={item.id}
                        onChange={setQuestionsToRepeatValue}
                        placeholder="eg. 3"
                    />
                    <Button
                    ...
                    {doubleLoop === true && (
                        <h5 className={styles.error}>
                            ERROR! Your repeat group contains another repeat
                            group, your form will break!
                        </h5>
                    )}
                    {Number.isNaN(numberQuestionsToRepeat) && (
                        <h5 className={styles.error}>
                            ERROR! Number of Previous Questions to Repeat must
                            be a Number!
                        </h5>
                    )}
                    {repeatQuestionsExceeded === true && (
                        <h5 className={styles.error}>
                            ERROR! You have repeated more questions than exist
                            prior to the loop! Change value now!
                        </h5>
                    )}
                    {loopCount > 1 && (
                        <h5 className={styles.error}>
                            ERROR! There should be only one geolocation field
                            per form. Remove one before submission.
                        </h5>
                    )}
                    {questionsToRepeat.length > 0 && (
                        <div>
                            <h4>
                                Questions in Repeat Group (Values only reflect
                                questions created prior to entering value.
                                Re-enter number to see updated group)
                            </h4>
