import { Button, Stack } from 'app/components/design-system/elements'
...
const Select = (props) => {
    ...
    const editOption = async (event, questionId, valueToChange) => {
        const { value, id } = event.target
        const textKeyValue = value.replace(
            /[`~!@#$%^&*()+=|}[{'";:?.>,<\\|\]/]+|_/g,
            ''
        )

        const elementsFormIndex = formItems.findIndex(
    ...
    const editTextOption = (optionId, val) => {
        const elementsIndex = options.findIndex(
